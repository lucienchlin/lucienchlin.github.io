...
3. Furthermore, Apryse states on its website that [the AGPL-3.0 licensees may not deploy iText/iTextSharp on a network without disclosing the full source code of their own applications under the AGPL license. Furthermore, it requires the licensees must distribute all source code, including their own product and web-based applications.](https://itextpdf.com/how-buy/AGPLv3-license)

using AGPL-3.0 licensed iText/iTextSharp software, all self-developed applications provided as web-based services, including other web-based programs, must also be released under AGPL-3.0.
